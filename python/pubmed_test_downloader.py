#!/usr/bin/env python3
"""
PubMed Test Downloader for BeforeDoctor
Downloads a small sample of pediatric studies for testing
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('pubmed_test_download.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class PubMedTestDownloader:
    """Downloads a small sample of PubMed pediatric studies for testing"""
    ...
    def __init__(self):
        ...
        # Reduced search queries for testing
        self.search_queries = [
    ...
    def download_test_studies(self) -> List[Dict[str, Any]]:
        """Download a small sample of studies for testing"""
        all_studies = []
        ...
        logger.info(f"Downloaded {len(self.studies)} unique test studies")
    ...
    def save_to_json(self, filename: str = None) -> str:
        ...
        if not filename:
            ...
            filename = f"pubmed_test_studies_{timestamp}.json"
        ...
        logger.info(f"Saved {len(self.studies)} test studies to {filename}")
        return filename
    ...
    def export_for_flutter(self, output_dir: str = "../beforedoctor/assets/data") -> str:
        ...
        output_file = os.path.join(output_dir, "pubmed_test_studies.json")
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(flutter_data, f, indent=2, ensure_ascii=False)
        
        logger.info(f"Exported Flutter-compatible test data to {output_file}")
        return output_file

def main():
    """Main function to download test PubMed datasets"""
    logger.info("Starting PubMed test dataset download for BeforeDoctor")
    
    downloader = PubMedTestDownloader()
    
    try:
        # Download test studies
        studies = downloader.download_test_studies()
        ...
        logger.info(f"Test download completed successfully!")
        ...
        logger.error(f"Error during test download: {e}")
        raise
